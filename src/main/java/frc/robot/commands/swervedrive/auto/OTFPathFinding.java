...
public class OTFPathFinding {
	...
	public static Command goToNearestReef(SwerveSubsystem swerveSubsystem) {
		...
		Angle angleToReef = Rotations.of(swerveSubsystem.getPose().getTranslation().minus(reef).getAngle().getRotations());
		...
		Pose2d clampedPoseLeft = new Pose2d(
			...
			Rotation2d.fromRadians(clampedAngle.in(Radians) + Math.PI)
		);
